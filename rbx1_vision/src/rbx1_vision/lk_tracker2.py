...
class LKTracker2(GoodFeatures):
    def __init__(self, node_name):
        super(LKTracker2, self).__init__(node_name)
    ...
    def process_image(self, cv_image):
        try:
            ...
            if not self.is_rect_nonzero(self.track_box) :
                print " rect nonzero"
            if self.track_box is None or not self.is_rect_nonzero(self.track_box):
                ...
                if self.originKeypoints is None :
                    self.originKeypoints = self.keypoints
    ...
    def track_keypoints(self, grey, prev_grey):
        ...
        if len(self.keypoints) < 2:
            self.keypoints = self.originKeypoints
